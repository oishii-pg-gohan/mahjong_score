...
import 'package:mahjong_score/view/main/conditions/yakus/yakus_yakuman/yakus_selector_yakuman.dart';
import 'package:mahjong_score/view/states/yakus/selected_yakus_provider.dart';

class YakusOpenButtonYakuman extends ConsumerWidget {
  const YakusOpenButtonYakuman({super.key});
  @override
  Widget build(BuildContext context, WidgetRef ref) {
    List<YakuId> selectedYakuIds = ref.watch(selectedYakusProvider);
    bool selected = selectedYakuIds
        .where((element) => yakuIdsYakuman.contains(element))
        .isNotEmpty;

    return YakusOpenButton(
      text: '役満',
      selected: selected,
      yakusList: const YakusYakuman(),
    );
  }
}
